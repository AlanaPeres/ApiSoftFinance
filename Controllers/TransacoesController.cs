...
namespace ApiSoftFinance.Controllers
{
    ...
    public class TransacoesController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> PostAsync(Transacao transacao, string cpfDestino, string cpf)
        {
            if(transacao.TipoTransacao == 'T')
            {
                try
                {
                    ...
                    if (sender.Saldo < transacao.Valor) return StatusCode(StatusCodes.Status203NonAuthoritative, "Transação não autorizado, saldo insuficiente.");
